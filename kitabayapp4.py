...
# Modified kitabay function to accept Amazon image URL
def kitabay(session, headers, book_name, amazon_image_url="https://source.unsplash.com/random/300x400/?book"):
    ...
    try:
        ...
        if not book_link:
            ...
            return book_name, "Unknown", "Unknown", amazon_image_url, "kitabay", 0.0, 0.0, "No description available", "Unknown", "Unknown", "Unknown"
        ...
        # Use Amazon image URL instead of extracting from Kitabay
        image_url = amazon_image_url
        logger.info(f"Using Amazon image URL for Kitabay: {image_url}")
...
# Modified scrape_book function to pass Amazon image URL to kitabay
def scrape_book(book_name):
    session = requests.Session()
    data = []
    
    try:
        # Scrape from Amazon first to get the image URL
        amazon_data = amazon(session, headers, book_name)
        logger.info(f"Amazon data: {amazon_data[0]}, price: {amazon_data[5]}")
        
        # Get the Amazon image URL to pass to kitabay
        amazon_image_url = amazon_data[3]
        ...
        # Pass the Amazon image URL to kitabay
        kitabay_data = kitabay(session, headers, book_name, amazon_image_url)
